...
public interface InterviewDao extends JpaRepository<Interview,Integer> {
    ...
    List<Interview> findAllByApplicationApplicantId(int id);

    List<Interview> findAllByApplicationApplicantIdAndEndTimeBefore(int id, LocalDateTime now);

    List<Interview> findAllByApplicationApplicantIdAndStartTimeAfter(int id, LocalDateTime now);
